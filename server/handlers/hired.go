...
func (h *handlerHired) CreateHired(c echo.Context) error {
	...
	seller_id, _ := strconv.Atoi(request.SellerID)
...
func (h *handlerHired) FindOfferByLogin(c echo.Context) error {
	userLogin := c.Get("userLogin")
	userId := userLogin.(jwt.MapClaims)["id"].(float64)

	offer, err := h.HiredRepository.FindOfferByLogin(int(userId))
	if err != nil {
		return c.JSON(http.StatusInternalServerError, dto.ErrorResult{Code: http.StatusBadRequest, Message: err.Error()})
	}

	return c.JSON(http.StatusOK, dto.SuccessResult{Code: http.StatusOK, Data: hireddto.HiredResponse{Hired: offer}})
}

func (h *handlerHired) FindOrderByLogin(c echo.Context) error {
	userLogin := c.Get("userLogin")
	userId := userLogin.(jwt.MapClaims)["id"].(float64)

	order, err := h.HiredRepository.FindOrderByLogin(int(userId))
	if err != nil {
		return c.JSON(http.StatusInternalServerError, dto.ErrorResult{Code: http.StatusBadRequest, Message: err.Error()})
	}

	return c.JSON(http.StatusOK, dto.SuccessResult{Code: http.StatusOK, Data: hireddto.HiredResponse{Hired: order}})
}

func (h *handlerHired) UpdateHired(c echo.Context) error {
	id, _ := strconv.Atoi(c.Param("id"))

	request := new(hireddto.UpdateHireRequest)
	if err := c.Bind(request); err != nil {
		return c.JSON(http.StatusBadRequest, dto.ErrorResult{Code: http.StatusBadRequest, Message: err.Error()})
	}
	...
	return c.JSON(http.StatusOK, hireddto.HiredResponse{Hired: data})
}
